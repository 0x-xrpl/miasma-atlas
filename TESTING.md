...
Use this checklist to verify the current submission state.
...
2. Run `npm run dev`.
3. Open the app.
4. Confirm the first screen reads as MIASMA.
5. Confirm the incident block shows `BLOCKED`.
6. Confirm the memory path mentions `vendor_policy_v3.txt -> payment_rules.md -> send_usdc`.
7. Confirm the verifier runs before execution and `fundsMoved` stays `0`.
8. Confirm the skill firewall shows `send_usdc`, simulated shadow execution, real execution blocked, and `Funds moved: 0`.
...
14. Confirm the receipt panel is a local sample implementation and does not imply an on-chain mint already occurred.
